...
def main():
    logger.info("Starting Universe Conditional Alpha Discovery (Phase 2.1)")
    ...
    for ticker in universe:
        ...
        if not os.path.exists(target_path):
            logger.warning(f"Skipping {ticker}, data missing.")
            continue
        ...
        if model:
            ...
            # FWER Multiple Testing context (simulated Bonferroni across 20 interactions)
            # In a true pipeline, we'd run OLS for all 20 and apply multipletests
            ...
            try:
                ...
                if interaction_term in res.tvalues:
                    ...
                    if fwer_p < 0.05:
                        ...
                        if oos_res['stable']:
                            stock_verdict = f"Robust Conditional Alpha ({interaction_term})"
                        else:
                            stock_verdict = f"Unstable OOS ({interaction_term})"
                    else:
                        stock_verdict = f"Rejected FWER ({interaction_term}, p={fwer_p:.3f})"
            except Exception as e:
                logger.error(f"OLS failed for {ticker}: {e}")
    ...
    # Run ML Discovery on Pooled Universe
    logger.info("Running XGBoost on Pooled Universe...")
    feature_cols = [c for c in pooled_df.columns if c.startswith('regime_') or c.startswith('is_') or c.startswith('day_')]
    ...
    if model:
        ...
        # Need firm fixed effects in pooled model (simulated via C(Firm))
        pooled_formula = f"ResidualReturn ~ C(Firm) + {weekday} * {condition}"
        try:
            res = estimate_panel_effects(pooled_df, pooled_formula)
            if interaction_term in res.tvalues:
                ...
                oos_res = walk_forward_validation(pooled_df, pooled_formula, seasonal_aware=True)
                logger.info(f"Pooled OOS Validation: {oos_res['reason']}")
                
                if fwer_p < 0.05 and oos_res['stable']:
                    logger.info("VERDICT: Sector-Wide Robust Conditional Alpha Detected.")
                else:
                    logger.info("VERDICT: Sector-Wide Conditional Alpha Rejected.")
        except Exception as e:
